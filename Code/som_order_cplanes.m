...
if ischar(simil), 
  ...
  case {'corr','abs(corr)','umat'}, 
    A = zeros(dim);
    me = zeros(1,dim);
    for i=1:dim, me(i) = mean(D(isfinite(D(:,i)),i)); D(:,i) = D(:,i) - me(i); end  
    for i=1:dim, 
      for j=i:dim, 
...
switch proj, 
 case 'SOM',
  sM2 = som_make(A,'munits',mu,'rect','tracking',0);
  bm  = assign_unique_bm(sM2,A);
  Co  = som_unit_coords(sM2);
  P   = Co(bm,:);

 case 'ring', 
  sM2 = som_make(A,'msize',[1 2*dim],'cyl','rect','tracking',0);
  bm  = assign_unique_bm(sM2,A);
  Co  = som_unit_coords(sM2);
  P   = Co(bm,[1 3]);   
  
 case 'order',
  sM2 = som_make(A,'msize',[1 2*dim],'tracking',0);
  bm  = assign_unique_bm(sM2,A);
  [dummy,i] = sort(bm); 
  [dummy,P] = sort(i);
